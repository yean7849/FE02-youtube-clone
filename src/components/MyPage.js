  ...
  return (
    <div className="mypage" id="profilewrap">
      ...
      <img className="userImage" src={UserProfile} alt="프로필 이미지" />
        ...
        <span className="MyPage__name" style={{ fontSize: '30px', fontWeight: 'bold'}}>{data.name}</span>
        <div className="channelInfo">
          <span style={{ fontSize: '15px'}}>{data.userChannelID}</span>
          <span>채널 보기</span>
        </div>
        <div className="accountAction" style={{ marginTop: '15px', marginLeft: '-10px'}}>
          <button>계정 전환</button>
        ...
        <hr/>
        <MyPageRow title="좋아요 표시한 동영상" Icon={ThumbUpAltOutlinedIcon} />
        <hr/>
        <MyPageRow title="내 클립" Icon={ContentCutOutlinedIcon} />
        <p>마음에 드는 순간을 잘라서 공유해 보세요. 바로 여기에 목록이 표시됩니다.</p>
      </div>
      </div>
    </div>
  );
